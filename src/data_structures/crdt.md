...
class ORSet:
    ...
    """
    def __init__(self):
        self.elements = {}       # {element: {unique_tag, ...}}  (add-set)
        self.tombstones = set()  # removed tags (so merges don't resurrect them)
    ...
    def _live_tags(self, element):
        """Tags for element that have not been tombstoned."""
        return self.elements.get(element, set()) - self.tombstones

    def remove(self, element):
        """Remove element by tombstoning all *observed* tags - $O(k)$.
        Tombstoning (not clearing) is what makes removes survive a later
        merge with a replica that still holds the old tag. Concurrent adds
        create fresh, non-tombstoned tags, so add-wins still holds.
        """
        if element in self.elements:
            self.tombstones |= self.elements[element]

    def contains(self, element):
        """Check if element exists - $O(k)$"""
        return len(self._live_tags(element)) > 0
    ...
    def merge(self, other):
        ...
        for element in all_elements:
            # Union of all tags for each element (live and tombstoned)
    ...
    def get_elements(self):
        """Get all elements in set - $O(m \cdot k)$"""
        return {elem for elem in self.elements if self._live_tags(elem)}
...
class ORSetWithGC(ORSet):
    ...
    def garbage_collect(self, cutoff_timestamp):
        """
        Drop elements with no live tags and discard their tombstoned tags - $O(m)$
        ...
        for element in list(self.elements.keys()):
            ...
            if not live:  # All tags removed
                self.tombstones -= self.elements[element]
                del self.elements[element]
            else:
                self.elements[element] = live
```
