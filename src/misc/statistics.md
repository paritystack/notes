...
- Diminishing returns: p99 í p99.9 much harder than p50 í p90
...
- 68% of values within 1√ of mean
- 95% of values within 2√ of mean
...
API latency:
- Service A: mean=50ms, √=5ms (very consistent)
- Service B: mean=50ms, √=100ms (wildly unpredictable)
...
- Mean = Median = Mode
- Defined by mean (º) and standard deviation (√)

**Why It's Everywhere**:
- **Central Limit Theorem**: Average of many independent random variables í normal
...
Server gets average 5 requests/second (ª=5)
...
**Key Insight**: Larger sample í narrower interval í more precise estimate

### Hypothesis Testing

**The Question**: Is what I'm seeing real, or just random chance?

**The Null Hypothesis** (HÄ): The boring explanation
- "No difference"
- "No effect"
- "Just randomness"

**Alternative Hypothesis** (HÅ): The interesting claim
...
- Slope = 2.5
- Interpretation: Each $1 in ads í $2.50 in sales (approximately)
...
**Action Items**:
- p99 > 1s í investigate
- p99 increasing í system degrading
- p50 vs p99 ratio > 10 í tail latency problem
...
- Multiple testing: 20 tests í 1 will be "significant" by chance
...
- Provisioning for average í p99 users suffer
- Provision for p99 í acceptable worst-case
...
1. **Mean hides outliers** í Use percentiles
2. **p99 matters** í 1% of users = thousands of people
3. **Correlation ` Causation** í Always question
4. **p-values misunderstood** í Report CI too
5. **Variance matters** í Same mean, different experience
6. **Context critical** í Numbers meaningless without it
7. **Long tails everywhere** í Normal distribution rare in real world
...
**For Decision Making**:
- Larger sample í more confidence
